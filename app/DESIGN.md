...
> **Note:** This document is a historical design record from the initial build, written around an EHS (health & safety) program as the driving use case. The application has since been generalized to support any employee-facing program — see [`README.md`](./README.md) for the current, use-case-agnostic overview. Some sections (e.g. §11 Authentication/Authorization) describe design decisions that were later reversed; they are kept for historical context only and do not reflect current behavior (auth/RBAC/business-unit isolation were removed).
